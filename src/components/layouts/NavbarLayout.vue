  ...
  <div class="min-h-screen bg-dashboard-bg">
    ...
    >
      <div class="flex h-full flex-col bg-sidebar-bg shadow-sidebar">
        <!-- Logo Section -->
        <div class="flex h-16 items-center justify-between px-6 border-b border-neutral-700">
        ...
        <div class="p-4 border-t border-neutral-700">
          <div class="flex items-center space-x-3 rounded-lg bg-sidebar-hover p-3">
            <div class="relative">
              <img 
                v-if="userPhotoURL" 
                :src="userPhotoURL" 
              ...
              />
              <div v-else class="h-8 w-8 rounded-full bg-primary-500 flex items-center justify-center">
                <User class="h-4 w-4 text-white" />
              </div>
              <!-- Role indicator -->
              <div class="absolute -bottom-1 -right-1 w-3 h-3 bg-success-500 border border-sidebar-hover rounded-full"></div>
            </div>
            <div class="flex-1 min-w-0">
              ...
              <div class="flex items-center space-x-2">
                ...
                <span class="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-success-100 text-success-800">
    ...
    <div class="lg:pl-64">
      <!-- Top Header -->
      <header class="sticky top-0 z-40 bg-white border-b border-dashboard-border">
        <div class="flex h-16 items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <div class="flex items-center space-x-4">
            <!-- Notifications -->
            <button class="relative p-2 rounded-full hover:bg-neutral-100 text-neutral-600">
              <Bell class="h-5 w-5" />
              <span class="absolute -top-1 -right-1 h-4 w-4 bg-error-500 rounded-full flex items-center justify-center">
              ...
              >
                <div class="px-4 py-3 border-b border-dashboard-border">
                  <div class="flex items-center space-x-3">
                    ...
                    />
                    <div v-else class="h-10 w-10 rounded-full bg-primary-500 flex items-center justify-center">
                      <User class="h-5 w-5 text-white" />
                    </div>
                    <div class="flex-1 min-w-0">
                      <p class="text-sm font-medium text-dashboard-text truncate">{{ userDisplayName }}</p>
                      <p class="text-xs text-dashboard-text-light truncate">{{ userEmail }}</p>
...
const navigationItems = [
  ...
  { name: 'Manajemen User', path: '/users', icon: UserCog },
]
...
const userDisplayName = computed(() => userDisplayInfo.value?.displayName || 'Admin')
const userEmail = computed(() => userDisplayInfo.value?.email || 'admin@example.com')
